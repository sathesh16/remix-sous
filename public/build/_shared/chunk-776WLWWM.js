...
// app/utils/get-asset-url.js
function getAssetURL(id) {
  if (!id)
    return null;
  return `${URL}/assets/${id}`;
}
var encode = (str) => encodeURIComponent(str).replace(/\-/g, "%2D").replace(/\_/g, "%5F").replace(/\./g, "%2E").replace(/\!/g, "%21").replace(/\~/g, "%7E").replace(/\*/g, "%2A").replace(/\'/g, "%27").replace(/\(/g, "%28").replaceAll("&", "%26").replaceAll("#", "%23").replace(/\)/g, "%29");
var decode = (str) => decodeURIComponent(
  str.replace(/\\%2D/g, "-").replace(/\\%5F/g, "_").replace(/\\%2E/g, ".").replace(/\\%21/g, "!").replace(/\\%7E/g, "~").replace(/\\%2A/g, "*").replace(/\\%27/g, "'").replace(/\\%28/g, "(").replaceAll("&amp;", "&").replaceAll("%23", "#").replaceAll("&quot;", '"').replace(/\\%29/g, ")")
);
{
}
...
//# sourceMappingURL=/build/_shared/chunk-776WLWWM.js.map
