...
import { createUser, readUsers, uploadFiles } from '@directus/sdk'
...
export async function action({ request }) {
    ...
    for (const image of images) {
        if (!image || typeof image === "string" || image.size === 0) continue;
    ...
    try {
        ...
        console.log("Directus createUser error:", err);

        return json({
            toast: {
                type: "error",
                message: err?.errors?.[0]?.message || "There is a problem in saving user",
            }
        });
    }
